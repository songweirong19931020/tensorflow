CREATE OR REPLACE FUNCTION "dwd"."fun_dwd_DI0005_d"("v_start_date" varchar, "v_end_date" varchar)
      RETURNS "pg_catalog"."varchar" AS $BODY$
    /***
    函数名称：住院患者患者住院患者是否微创手术患者(0 否 1是)（国家标准）   
        作用：统计某天住院患者住院患者是否微创手术患者(0 否 1是)（国家标准）   
      开发人：leslie 2020-08-17
    命名规范：FUN_模型层级(DWD或者DW)_KPI编码_日期类型D或者M，D表示按天统计，M表示按月统计
     KPI编码：DI0005  根据原子指标编码规划来的
    ...
    ***/
    ...
    delete from his_bi.dwd_inp_medical_d  where st_date = c_daylist.day_id and key = 'DI0005';
 
  INSERT into his_bi.dwd_inp_medical_d (key,value,patient_id,visit_id,pai_visit_id,insert_date,
                    remark,st_date) 
  select distinct 'DI0005',
     1,
      a.bah as patient_id,
     a.zycs as visit_id,
     pt.pai_visit_id,
   ...
   inner join ods.his_pts_pai_visit pt on a.bah = pt.patient_id and a.zycs = pt.visit_id
